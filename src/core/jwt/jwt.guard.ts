...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!user?.role) {
      throw new ForbiddenException('User roles not found | Access denied');
    }
    ...
    if (!hasRole) {
      throw new ForbiddenException(
        'Sorry,Wrong user role | Access denied| we can access this route use proper role, otherwise contact developer',
      );
    }
